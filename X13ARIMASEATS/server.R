...
function(input, output) {
  ...
  output$prueba <- renderText(file.access("/cloud/project/x13as.exe", mode = 1))
  observeEvent(input$run, {
    ...
    if (is.null(input$SPC_file) | is.null(input$DAT_file)) {
      ...
      withProgress(message = 'Calculation in progress...', style = "notification", value = 0.35, {
        ...
        system2("x13as.exe", args = "temp/SPC_temp -s", stdout = TRUE, stderr = TRUE)
        ...
        if (length(condition) < 10) { #Este valor es arbitrario: (10)
          ...
          system2("x13as.exe", args = "temp/SPC_graficas -s", stdout = TRUE, stderr = TRUE)
